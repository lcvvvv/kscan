...
var KeywordFingers keywordFingers
var HashwordFingers hashFingers

type keywordFinger struct {
	Type, Cms, Keyword string
}

type keywordFingers struct {
	KeywordFingers []keywordFinger
}

type hashFinger struct {
	Hash, Cms string
}

type hashFingers struct {
	HashFingers []hashFinger
}

func LoadFinger() {
	KeywordFingers = loadKeywordFingers()
	HashwordFingers = loadHashFingers()
}

func loadKeywordFingers() keywordFingers {
	if misc.FileIsExist("fingers.json") {
		file, _ := os.Open("fingers.json")
		defer file.Close()
		...
		fmt.Printf("[*]成功读取关键字指纹数据%d条!!!\n", len(out.KeywordFingers))
		return out
	} else {
		fmt.Print("[-]未检测到存在fingers.json关键字指纹库，正在创建...\n")
		file, _ := os.Create("fingers.json")
		fmt.Print("[+]成功创建关键字指纹库：fingers.json\n")
...
func loadHashFingers() hashFingers {
	if misc.FileIsExist("faviconhashs.json") {
		file, _ := os.Open("faviconhashs.json")
		defer file.Close()
		decoder := json.NewDecoder(file)
		out := hashFingers{}
		err := decoder.Decode(&out)
		...
		fmt.Printf("[*]成功读取图标指纹数据%d条!!!\n", len(out.HashFingers))
		return out
	} else {
		fmt.Print("[-]未检测到存在faviconhashs.json关键字指纹库，正在创建...\n")
		file, _ := os.Create("faviconhashs.json")
		fmt.Print("[+]成功创建关键字指纹库：faviconhashs.json\n")
